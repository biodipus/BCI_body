...
%% Fig 4G
H = importdata('Hweight_Hold_bot50_for_plot.mat'); 
N = importdata('Nweight_Hold_bot50_for_plot.mat'); 
prob = cat(3, H.prob2, N.prob2); % prob:hand
...
for f = 159 % Fig 4F example
    file = files(f).name; 
    load(['.\Fig2\Monkey_data\Neuorns\N\',file]);
...
%     [~,~,ind] = histcounts(drf,edges); ind = [types(:,1),types(:,4),ind]; [~,loc] = ismember(ind,conds,'rows'); % [cond,rot,drf_idx];
    ...
    cond3 = (types(:,1)==3 & types(:,2)==6); cond3 = [types(cond3,4),drf(cond3),pr1(cond3)]; %[rot,drf,pr1];
