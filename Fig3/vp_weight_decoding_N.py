...
vpweight_filename = 'NweightHoldThresh3trls.mat'
prob = sio.loadmat(os.path.join(path, vpweight_filename))
vpweight = prob['prob'][:,:,:,0]
conds = prob['conds']

subselect = sio.loadmat(os.path.join(path, 'VPTselID_N_HW.mat'))['id3'][:,0]
...
for i in subselect:
    ...
    for j in range(len(rotation)):
        sort_ind = np.argsort(dft_sub[:, j+1], 0)
        dft_rot = dft_sub[sort_ind, j+1]
        pc1_rot = pc1_sub[sort_ind, j+1]
...
vpweight_sd = vpweight[:, :, :] # list(range(0,9))+list(range(20,29))
...
for train, test in rkf.split(np.arange(trial)):
    ...
    y_train = pc1_all_df.loc[:, 'label'].to_numpy().reshape(-1,29)[train,:].reshape(-1)
    ...
    y_test = pc1_all_df.loc[:, 'label'].to_numpy().reshape(-1,29)[test,:].reshape(-1)
    ...
    clf = SVC(kernel='linear', C=1, class_weight={3:0.4, 5:0.4, 6:0.2, 7:0.1}) 
    ...
    print(k)
    if k % 3 ==0:
        print(np.mean(balanced_acc[-3:]))
    
predict_acc_mean = predict_acc.mean(2)
print(np.mean(scores))
print(np.mean(balanced_acc))
#pickle.dump({'model': clf, 'train_ind': train_ind, 'test_ind': test_ind, 
#             'predict_acc': predict_acc, 'scores': scores, 'confu_mat': confu_mat, 'balanced_acc' :balanced_acc}, 
#            open(path + '\\decoding\\SVC_linear_vpselect102_tuning_3trl_8class_hold3.pkl', 'wb'))
### plot accuracy ####
