...
for f = 162 % Fig 3B:162 3C:64; Fig S5:195;
%     subplot(10,10,f)
    figure(2)
    file = files(f).name; 
...
%     type = data.type; % theta = unique(type(:,[2,8]),'rows'); 
    ...
    rots = unique(types(:,4)); tgts = unique(types(:,3)); 
...
for f = 162
    file = files(f).name; 
    load(['D:\Projects\MI\BCI_data20230118\Monkey_data\Neuorns\H\',file]); 
    dat = data.all; info = data.info; spks = data.spks2;
    if isempty(dat)
        continue;
    end    
    type = data.type; % theta = unique(type(:,[2,8]),'rows'); 
...
%         spk = spk(randperm(length(spk),30));
        for trl = 1:length(spk)
            if length(spk{trl})>=5
            spk1 = spk{trl}; spk1=spk1(spk1<=0.2); x = repmat(spk1,1,2); % x=x+itg*2; 
